...
def evaluate(model, loader, device):
    ...
    for xb, yb in loader:
        ...
        preds.append(yhat)
        ys.append(yb.numpy())
    y_true = np.concatenate(ys) + 1   # back to 1..6
...
def main():
    ...
    if args.split == "standard":
        ...
        model = CNN1D(n_channels=n_channels, n_classes=6, dropout=args.dropout).to(device)

        criterion = nn.CrossEntropyLoss()
        optim = torch.optim.Adam(model.parameters(), lr=args.lr)

        best_metric = -1.0
        best_path = f"outputs/results/cnn1d_{args.split}_{args.sensors}_seed{args.seed}.pt"
        ...
        # SUBJECT split: combine then split by subjects
